...
class AVLTree3 {                            // início da classe que implementa a árvore AVL
    ...
    static class Node {                     // classe interna que representa um nó da árvore
        int valor;                          // campo que guarda o valor inteiro armazenado no nó
    ...
    Node inserir(Node node, int valor) {    // método recursivo que insere um valor em um nó/subárvore
    ...
    public void inserir(int valor) {
        System.out.println("Inserção valor: " + valor);
    ...
    // Buscar valor
    public boolean buscar(Node node, int valor) { // busca recursiva de valor (começa geralmente pela raiz)
        if (node == null) return false;       // se nó é null, não encontrou
        if (valor == node.valor) return true; // se valor igual ao do nó, encontrou
        if (valor < node.valor) return buscar(node.esquerda, valor);
...
// Teste
public class ArvoreBalanceada3 {               // classe com método main para testar a AVLTree2
    public static void main(String[] args) {
        AVLTree3 arvore = new AVLTree3();     // cria uma nova árvore AVL vazia

        int[] valores = {10, 20, 30, 40, 50, 25}; // vetor de valores a serem inseridos
        for (int v : valores) {
            arvore.inserir(v);                // insere cada valor na árvore
        }
        ...
        System.out.println("\nPesquisa.....................");
        // Testando busca
        System.out.println("Buscar 25? " + arvore.buscar(arvore.raiz, 25));
        // imprime true se encontrou 25 na árvore

        System.out.println("Buscar 15? " + arvore.buscar(arvore.raiz, 15));
